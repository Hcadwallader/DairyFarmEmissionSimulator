const uploadInitialFarmData = (wb) => {
	var details = {};
	...
	do {
		...
		if (farmName != null) {
			...
			let amount = purchaseData.getRow(rowNumber).getCell('C')
				.value.result;
			const unit = purchaseData.getRow(rowNumber).getCell('D').value;
	...
	return details; // if the name is null assume we've reached the end
};
...
module.exports = uploadInitialFarmData;
